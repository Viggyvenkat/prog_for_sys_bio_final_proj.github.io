...
- *The Objective:* This project aims to integrate KF genomic data with SenNet tissue atlases to determine if germline variants associated with childhood cancer susceptibility (from KF) enrichment in pathways that regulate the cellular secretome or promote "dark" senescence in embryonic tissues (from SenNet).

### Aim 1: Map KF Susceptibility Variants to SenNet Regulatory Landscapes.
We will utilize the SenNet Atlas to identify cell-type-specific enhancers and open chromatin regions (via single-cell ATAC-seq data) unique to senescent populations. We will then perform an enrichment analysis of WGS data from KF childhood cancer cohorts to determine if non-coding risk variants are significantly concentrated within these senescent-specific regulatory elements.
- *Hypothesis:* Pediatric cancer risk variants are disproportionately located in regulatory regions that govern the senescent cell state during early development.

### Aim 2: Functional Validation of the Secretome in Pediatric Oncogenesis.
Using SenNet secretome profiles, we will identify pro-inflammatory signaling molecules (SASP factors) that are upregulated in tissues prone to structural defects. We will cross-reference these factors with KF phenotypic data to identify "secretome signatures" that correlate with rapid cancer progression in children with pre-existing birth defects.
- *Hypothesis:* A "pro-tumorigenic" secretome, characterized by specific SASP factors, creates a permissive microenvironment for cells carrying KF-identified driver mutations to expand into malignancies.
